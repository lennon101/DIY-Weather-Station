...
const int rain_gauge_pin = 16;  // pin used for the tipping bucket rain gauge - attached to internal pull-up resistor
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  while (out_of_range(temp, 0, 60) || out_of_range(pressure, 800, 1200) || out_of_range(rh, 0, 100)) {
    //get_bme_values(temp, pressure, rh);
    if (attempts_left-- <= 0) {
...
void setup_rain_guage() {
  pinMode(rain_gauge_pin, INPUT);
  attachInterrupt(rain_gauge_pin, Ext_INT1_ISR, FALLING);
}
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {  //Connected to WiFi
    ...
    client.loop();
    client.publish("esp32/out/keepalive", "keepalive");  // this is done to force the connection to remain open.
    delay(1000);
